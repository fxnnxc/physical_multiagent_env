...
class Observation_1:
    def __init__(self, num_targets):
        ...
        self.observation_space.update({'8x8_observation' :Box(low=-np.inf, high=np.inf, shape=(8,8))})
    ...
    def observation_fn_1(agent_obs, test_env=None, **kw):
        ...
        for a in agent_obs.keys():
            ...
            for i, target in enumerate(env.objects['target']):
                ...
                new_obs[a]['8x8_observation'] = np.zeros((8,8)) # relative position and dangerous_degree

                for obj_type, obj_list in env.objects.items():
                    for obj in obj_list:
                        distance = agent.distance(obj)
                        if distance < 4:
                            position = np.ceil(agent.relative_position(obj)-4)+1
                            position = position.astype(int)
                            dg = dangerous_degree(agent.velocity, obj.velocity, obj.globalScaling)
                            dg = max(new_obs[a]['8x8_observation'][position[0], position[1]], dg)
                            new_obs[a]['8x8_observation'][position[0], position[1]] = dg
                            #new_obs[a]['4x4_observation'][position[0], position[1],:] = agent.relative_velocity(obj)

        return new_obs


def dangerous_degree(v1, v2, globalScaling):
    v1, v2 = np.array(v1), np.array(v2)
    return np.linalg.norm(1/(v1+v2+1)) * globalScaling * (1 + np.linalg.norm(v1) + np.linalg.norm(v2))
